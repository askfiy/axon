...
Revision ID: 0d1e4efca26b
Revises: 
Create Date: 2025-07-22 23:05:00.809386
...
def upgrade() -> None:
    ...
    op.create_table('tasks_histories',
    sa.Column('task_id', sa.BigInteger(), nullable=False, comment='关联任务ID'),
    ...
    sa.Column('think', sa.Text(), nullable=False, comment='Agent 的思考过程'),
    sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False, comment='创建时间'),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True, comment='更新时间'),
    sa.Column('deleted_at', sa.DateTime(timezone=True), nullable=True, comment='删除时间'),
    ...
    op.create_index('idx_tasks_history_task_state', 'tasks_histories', ['task_id', 'state'], unique=False)
    op.create_index(op.f('ix_tasks_histories_created_at'), 'tasks_histories', ['created_at'], unique=False)
    op.create_index(op.f('ix_tasks_histories_is_deleted'), 'tasks_histories', ['is_deleted'], unique=False)
    op.create_index(op.f('ix_tasks_histories_state'), 'tasks_histories', ['state'], unique=False)
    op.create_index(op.f('ix_tasks_histories_task_id'), 'tasks_histories', ['task_id'], unique=False)
    op.create_index(op.f('ix_tasks_histories_updated_at'), 'tasks_histories', ['updated_at'], unique=False)
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f('ix_tasks_histories_updated_at'), table_name='tasks_histories')
    op.drop_index(op.f('ix_tasks_histories_task_id'), table_name='tasks_histories')
    op.drop_index(op.f('ix_tasks_histories_state'), table_name='tasks_histories')
    op.drop_index(op.f('ix_tasks_histories_is_deleted'), table_name='tasks_histories')
    op.drop_index(op.f('ix_tasks_histories_created_at'), table_name='tasks_histories')
    op.drop_index('idx_tasks_history_task_state', table_name='tasks_histories')
    op.drop_table('tasks_histories')
