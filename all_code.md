...
class PaginationResponse(BaseHttpResponseModel):
    ...
    result: list[PaginationSerializer] = Field(
...
class Paginator(
    ...
    response: PaginationResponse[PaginationSerializer] = Field(
...
class TasksAuditRepository(BaseCRUDRepository[TasksAudit]):
    async def upget_tasks_audit_pagination(
        self,
        task_id: int,
        paginator: Paginator,
        joined_loads: list[InstrumentedAttribute[Any]] | None = None,
...
async def upget_tasks_pagination(paginator: Paginator) -> Paginator:
    async with get_async_session_direct() as session:
        tasks_repo = TasksCRUDRepository(session=session)
        return await tasks_repo.upget_tasks_pagination(paginator=paginator)
...
async def upget_tasks_history_pagination(
    ...
    async with get_async_session_direct() as session:
        ...
        return await tasks_history_repo.pget_tasks_history_pagination(
            task_id=task_id,
            paginator=paginator,
        )
